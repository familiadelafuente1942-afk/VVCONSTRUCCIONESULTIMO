...
// VERSION: v97 (particulares: inversion mensual promedio = total / meses de obra)
      ...
      return (<div key={p.id} style={{ background: T.bg, borderRadius: 12, padding: 13, marginTop: 12 }}>
        ...
        {costos.length > 0 && (() => { const sl = !!sinLote[p.id]; const fechaDe = (c) => c.fecha || (c.ts ? new Date(c.ts).toISOString().slice(0, 10) : ""); const corteISO = corte[p.id] || hoyISO(); const enCorte = (c) => { const d = fechaDe(c); return !d || d <= corteISO; }; const cf = costos.filter(c => !(sl && String(c.cat).toLowerCase() === "lote")).filter(enCorte); const loteArs = costos.filter(c => String(c.cat).toLowerCase() === "lote" && enCorte(c)).reduce((s, c) => s + arsUnif(c, cotU), 0); const porMes = {}; cf.forEach(c => { const k = mesKey(c); porMes[k] = (porMes[k] || 0) + arsUnif(c, cotU); }); const meses = Object.keys(porMes).sort(); const items = meses.map(k => ({ label: mesLbl(k), value: porMes[k] })); const totFlujo = meses.reduce((s, k) => s + porMes[k], 0); const inicioISO = p.inicio || (cf.length ? cf.map(fechaDe).filter(Boolean).sort()[0] : ""); const mesesEntre = (a, b) => { const x = new Date(a), y = new Date(b); if (isNaN(x.getTime()) || isNaN(y.getTime())) return 0; return (y.getFullYear() - x.getFullYear()) * 12 + (y.getMonth() - x.getMonth()) + 1; }; const mesesObra = inicioISO ? Math.max(1, mesesEntre(inicioISO, corteISO)) : Math.max(1, meses.length); const promedio = totFlujo / mesesObra; return (<div style={{ marginTop: 10, background: T.card, borderRadius: 11, padding: 12, border: `1px solid ${T.border}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, gap: 8, flexWrap: "wrap" }}>
            ...
            {loteArs > 0 && <button onClick={() => setSinLote(s => ({ ...s, [p.id]: !s[p.id] }))} style={{ background: sl ? T.accent : T.al, color: sl ? "#fff" : T.accent, border: `1px solid ${sl ? T.accent : T.border}`, borderRadius: 8, padding: "5px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>{sl ? "Sin lote ✓" : "Sacar lote"}</button>}
          </div>
          ...
          {items.length > 0 ? <><GraficoBarras titulo="" items={items} />
            ...
            {sl && loteArs > 0 && <div style={{ fontSize: 10.5, color: T.muted, textAlign: "right", marginTop: 3 }}>Lote excluido: {money(loteArs)}</div>}</> : <div style={{ fontSize: 11, color: T.muted, textAlign: "center", padding: "6px 0" }}>Sin inversión cargada hasta esa fecha.</div>}
        </div>); })()}
