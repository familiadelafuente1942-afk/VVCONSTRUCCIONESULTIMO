  ...
  function imprimir() {
    ...
    if (!esCosto) {
      ...
      if (adelantos.length > 0) {
        adelantoHtml = `<h2>Adelanto de certificados</h2><table><thead><tr><th>Obra</th><th class="rgt">Monto</th></tr></thead><tbody>${filasAdelanto}<tr class="big"><td class="rgt">SUBTOTAL ADELANTOS</td><td class="rgt" style="color:#DC2626">− ${money(totAdelantos)}</td></tr></tbody></table>`;
      }
      // 2) Ajuste obras sin certificación: va tercera, con la MISMA jerarquía visual que "Certificados".
      if (filasCobrado) {
        cobradoSinCertHtml = `<h2>Ajuste obras sin certificación</h2><table><thead><tr><th>Obra</th><th class="rgt">Monto</th></tr></thead><tbody>${filasCobrado}<tr class="big"><td class="rgt">SALDO SIN CERTIFICACIÓN</td><td class="rgt">${money(totCobrosSinCertTodo)}</td></tr></tbody></table>`;
      }
    }
