...
// VERSION: v72 (cada gasto con su cotizacion propia, unificacion opcional, default pesos)
    ...
    {propias.map(p => {
      ...
      return (<div key={p.id} style={{ background: T.bg, borderRadius: 12, padding: 13, marginTop: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          ...
          <button onClick={() => { const rt = {}; costos.forEach(c => { rt[c.cat] = (rt[c.cat] || 0) + num(c.montoArs); }); const arr = Object.entries(rt).filter(([, v]) => v > 0).sort((a, b) => b[1] - a[1]); const top = arr.slice(0, 8).map(([k, v]) => ({ label: k, value: v })); const ot = arr.slice(8).reduce((a, [, v]) => a + v, 0); if (ot > 0) top.push({ label: "Otros", value: ot }); setPdfHtml(reporteHTML(p.nombre + " — Obra particular", "", seccionesPropia(p), p.adjuntos, data.config, [{ tipo: "torta", titulo: "Costos por rubro", items: top }, { tipo: "barras", titulo: "Inversión vs resultado", items: [{ label: "Inversión", value: totArs, color: "#C2410C" }, { label: "Resultado", value: Math.max(0, resA), color: "#16A34A" }] }])); }} style={{ flex: 1, background: T.navy, color: "#fff", border: "none", borderRadius: 9, padding: "10px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>PDF</button>
        </div>
      </div>);
    })}
