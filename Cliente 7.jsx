...
// VERSION: v14 (avisos rojos en TODOS los iconos: obras nuevas, pedidos, mensajes, IA, formularios)
// ════════════════════════════════════════════════════════════════════
...
function ClienteApp() {
  ...
  useEffect(() => {
    ...
    async function tick() {
      ...
      if (!alive) return;
      if (rmp?.value && Number(rmpTs?.value || 0) > (lastWrite["vv_matpedidos"] || 0)) { try { lastWrite["vv_matpedidos"] = Number(rmpTs.value); setMatpedidos(JSON.parse(rmp.value)); } catch { } }
      if (rm?.value) {
        try {
