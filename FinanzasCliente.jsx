...
// Redeterminación mensual, encadenada: Saldo_final = (Saldo_inicial − Cobro_del_cliente_del_mes) × (1 + IPC_del_mes)
...
export default function App() {
  ...
  return (<div style={{ minHeight: "100vh", background: fondoDe(cfg), fontFamily: fuenteDe(cfg), width: "100%", color: T.text }}>
    ...
    <div style={{ background: `linear-gradient(180deg, #0E1B2B 0%, ${T.navy} 100%)`, color: "#fff", padding: "calc(14px + env(safe-area-inset-top)) 16px 16px", textAlign: "center" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <button onClick={actualizar} title="Actualizar" style={{ background: "rgba(255,255,255,.12)", border: "none", color: "#fff", borderRadius: 9, height: 34, padding: "0 12px", fontSize: 12.5, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>↻ {okMsg || (refrescando ? "..." : "Actualizar")}</button>
        <button onClick={() => setVerConfig(true)} title="Personalización" style={{ background: "rgba(255,255,255,.12)", border: "none", color: "#fff", borderRadius: 9, width: 34, height: 34, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>⚙︎</button>
      </div>
...
// Igual que GraficoTortaRubros/GraficoBarrasRubros pero como texto SVG
// (para pegar directo dentro de un PDF armado con string HTML, donde no
// se pueden usar componentes de React).
function svgTortaRubrosHTML(rubros, size = 170) {
  const total = rubros.reduce((s, r) => s + Math.max(0, num(r.pct)), 0) || 1;
  const cx = size / 2, cy = size / 2, rad = size / 2 - 4;
  let acumAngulo = -90;
  const paths = rubros.map((r, i) => {
    const pct = Math.max(0, num(r.pct)) / total;
    const angulo = pct * 360;
    const x1 = cx + rad * Math.cos((acumAngulo * Math.PI) / 180), y1 = cy + rad * Math.sin((acumAngulo * Math.PI) / 180);
    const finAngulo = acumAngulo + angulo;
    const x2 = cx + rad * Math.cos((finAngulo * Math.PI) / 180), y2 = cy + rad * Math.sin((finAngulo * Math.PI) / 180);
    const largeArc = angulo > 180 ? 1 : 0;
    const d = pct >= 0.999 ? `M ${cx} ${cy - rad} A ${rad} ${rad} 0 1 1 ${cx - 0.01} ${cy - rad} Z` : `M ${cx} ${cy} L ${x1} ${y1} A ${rad} ${rad} 0 ${largeArc} 1 ${x2} ${y2} Z`;
    acumAngulo = finAngulo;
    return `<path d="${d}" fill="${RUBRO_COLORS[i % RUBRO_COLORS.length]}" stroke="#fff" stroke-width="1.5"/>`;
  }).join("");
  return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">${paths}</svg>`;
}
function svgBarrasRubrosHTML(rubros, width = 320) {
  const ordenados = rubros.slice().sort((a, b) => num(b.pct) - num(a.pct));
  const max = Math.max(1, ...ordenados.map(r => num(r.pct)));
  const filaAlto = 24, labelAncho = 108, barMax = width - labelAncho - 44;
  const filas = ordenados.map((r, i) => {
    const w = (num(r.pct) / max) * barMax, y = i * filaAlto;
    const color = RUBRO_COLORS[rubros.indexOf(r) % RUBRO_COLORS.length];
    const nombreCorto = (r.nombre || "—").length > 16 ? (r.nombre || "").slice(0, 15) + "…" : (r.nombre || "—");
    return `<text x="${labelAncho - 6}" y="${y + filaAlto / 2 + 4}" text-anchor="end" font-size="10" fill="#5B6B7F">${nombreCorto}</text><rect x="${labelAncho}" y="${y + 4}" width="${Math.max(2, w)}" height="${filaAlto - 8}" rx="3" fill="${color}"/><text x="${labelAncho + w + 6}" y="${y + filaAlto / 2 + 4}" font-size="10" font-weight="700" fill="#1B3A5B">${num(r.pct).toFixed(1)}%</text>`;
  }).join("");
  return `<svg width="${width}" height="${ordenados.length * filaAlto}" viewBox="0 0 ${width} ${ordenados.length * filaAlto}">${filas}</svg>`;
}
function graficoRubrosPresupuestoHTML(rubros) {
  const activos = (rubros || []).filter(r => num(r.pct) > 0);
  if (activos.length === 0) return "";
  const referencia = activos.map((r, i) => `<div style="display:flex;align-items:center;gap:7px;margin-bottom:5px"><span style="width:10px;height:10px;border-radius:3px;background:${RUBRO_COLORS[i % RUBRO_COLORS.length]};flex-shrink:0;display:inline-block"></span><span style="font-size:11.5px;color:#3A4658">${r.nombre || "—"} <b style="color:#1B3A5B">${num(r.pct).toFixed(1)}%</b></span></div>`).join("");
  return `<h2>Incidencia de cada rubro</h2><div style="display:flex;gap:18px;align-items:center;flex-wrap:wrap;margin-bottom:14px"><div>${svgTortaRubrosHTML(activos, 150)}</div><div style="flex:1;min-width:150px">${referencia}</div></div><div style="margin-bottom:10px">${svgBarrasRubrosHTML(activos, 320)}</div>`;
}
// Gráfico de barras del CERTIFICADO: una barra por rubro, dividida entre lo
// ya certificado (acumulado, en dorado) y lo que resta (en gris) — para ver
// de un vistazo qué rubro está completo y cuál falta.
function svgBarrasCertificadoHTML(det, width = 320) {
  const filaAlto = 30, labelAncho = 110, barMax = width - labelAncho - 14;
  const filas = det.map((d, i) => {
    const y = i * filaAlto;
    const wAcum = (Math.min(100, d.pctAcum) / 100) * barMax;
    const wResta = barMax - wAcum;
    const nombreCorto = (d.nombre || "—").length > 17 ? (d.nombre || "").slice(0, 16) + "…" : (d.nombre || "—");
    return `<text x="${labelAncho - 6}" y="${y + filaAlto / 2 - 2}" text-anchor="end" font-size="10" fill="#5B6B7F">${nombreCorto}</text>
      <rect x="${labelAncho}" y="${y + 3}" width="${barMax}" height="14" rx="3" fill="#EEF1F5"/>
      <rect x="${labelAncho}" y="${y + 3}" width="${Math.max(0, wAcum)}" height="14" rx="3" fill="#B0894F"/>
      <text x="${labelAncho + barMax + 6}" y="${y + 14}" font-size="9.5" font-weight="700" fill="#1B3A5B">${d.pctAcum.toFixed(0)}%</text>`;
  }).join("");
  return `<svg width="${width + 34}" height="${det.length * filaAlto}" viewBox="0 0 ${width + 34} ${det.length * filaAlto}">${filas}</svg>`;
}
function graficoCertificadoHTML(det) {
  if (!det || det.length === 0) return "";
  return `<h2>Avance por rubro</h2><div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;font-size:10.5px;color:#5B6B7F"><span style="display:inline-flex;align-items:center;gap:5px"><span style="width:10px;height:10px;background:#B0894F;border-radius:2px;display:inline-block"></span>Certificado</span><span style="display:inline-flex;align-items:center;gap:5px"><span style="width:10px;height:10px;background:#EEF1F5;border-radius:2px;display:inline-block;border:1px solid #ddd"></span>Resta</span></div><div style="margin-bottom:10px">${svgBarrasCertificadoHTML(det)}</div>`;
}
...
function CertGeneral({ obras, data, save, certsDe, indices, modo }) {
  ...
  const [mes, setMes] = useState("");   // "" = todos los períodos juntos
  const mesSel = mes === "" || meses.includes(mes) ? mes : "";
  const todosPeriodos = mesSel === "";

  // cada renglón = un certificado (del mes elegido, o de todos si no hay filtro).
  // cliente → neto (lo que Belfast paga); costo → costo directo del período (lo que gastás).
  const filas = [];
  obras.forEach(o => {
    const cs = certsDe(o.id);
    cs.forEach((c, i) => {
      if (!todosPeriodos && mesDe(c.fecha) !== mesSel) return;
  ...
  function imprimir() {
    ...
    const periodoTxt = todosPeriodos ? "Todos los períodos" : mesLabel(mesSel);
    ...
    if (!esCosto) {
      ...
      // 1) Adelanto de certificados: sección propia, va segunda.
      if (adelantos.length > 0) {
        adelantoHtml = `<h2>Adelanto de certificados</h2><table class="t2"><colgroup><col/><col/></colgroup><thead><tr><th>Obra</th><th class="rgt">Monto</th></tr></thead><tbody>${filasAdelanto}<tr class="big"><td class="rgt">SUBTOTAL ADELANTOS</td><td class="rgt" style="color:#DC2626">− ${money(totAdelantos)}</td></tr></tbody></table>`;
      }
      // 2) Ajuste obras sin certificación: va tercera, con la MISMA jerarquía visual que "Certificados".
      if (filasCobrado) {
        cobradoSinCertHtml = `<h2>Ajuste obras sin certificación</h2><table class="t2"><colgroup><col/><col/></colgroup><thead><tr><th>Obra</th><th class="rgt">Monto</th></tr></thead><tbody>${filasCobrado}<tr class="big"><td class="rgt">SALDO SIN CERTIFICACIÓN</td><td class="rgt">${money(totCobrosSinCertTodo)}</td></tr></tbody></table>`;
      }
    }
    const totalFilaFinal = (!esCosto && (adelantos.length > 0 || granTotalFinal !== total)) ? `<tr class="grandtotal"><td class="rgt">GRAN TOTAL</td><td class="rgt">${money(granTotalFinal)}</td></tr>` : "";

    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${tituloDoc} ${periodoTxt}</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,Arial,sans-serif;color:#0F1B2D;padding:0 0 34px;line-height:1.5}.head{background:#0F1B2D;color:#fff;padding:20px 40px;border-bottom:4px solid #B0894F;display:flex;justify-content:space-between;align-items:center}.brand{font-size:22px;font-weight:800}.brand small{display:block;font-size:10px;color:#B0894F;letter-spacing:2px;margin-top:2px}.doc{text-align:right;font-size:11px;color:#cdd5e0}.doc b{display:block;font-size:15px;color:#fff}.wrap{padding:0 40px}.meta{display:flex;justify-content:space-between;margin:22px 0 6px;font-size:12.5px}.meta span{color:#5B6B7F}h2{font-size:12px;color:#5B6B7F;text-transform:uppercase;letter-spacing:1px;margin:20px 0 8px;border-bottom:1px solid #E3E8EF;padding-bottom:5px}p{font-size:12.5px;margin:8px 0}table{width:100%;border-collapse:collapse;font-size:12.5px;table-layout:fixed}th{background:#EAF0F7;color:#1B3A5B;text-align:left;padding:8px 10px;font-size:10.5px;text-transform:uppercase}td{padding:9px 10px;border-bottom:1px solid #EEF1F5}.ctr{text-align:center}.rgt{text-align:right}.big td{border-top:2px solid #0F1B2D;font-size:14px;font-weight:800;color:#1B3A5B;padding-top:11px;background:#F4F6F9}.grandtotal td{border-top:3px solid #0F1B2D;font-size:23px;font-weight:800;color:#1B3A5B;padding-top:14px;background:#F4F6F9}.foot{display:flex;justify-content:space-between;font-size:11px;color:#5B6B7F;margin-top:54px}.sign{width:240px;text-align:center}.sign .ln{border-top:1px solid #0F1B2D;padding-top:5px}.t2 col:first-child{width:75%}.t2 col:last-child{width:25%}</style></head><body><div class="head">${brandHtml}<div class="doc"><b>${tituloDoc}</b>Período: ${periodoTxt}<br>Fecha: ${fmtISO(hoyISO())}</div></div><div class="wrap"><div class="meta"><div><span>Comitente:</span> <b>${comitente}</b></div>${metaDer}</div><p>${introTxt}</p><h2>Certificados${todosPeriodos ? "" : ` del período`}</h2><table><thead><tr><th>Obra</th><th class="ctr">Certificado</th><th class="ctr">Fecha</th><th class="rgt">${colMonto}</th></tr></thead><tbody>${rows}<tr class="big"><td colspan="3" class="rgt">${totalLbl}</td><td class="rgt">${money(total)}</td></tr></tbody></table>${adelantoHtml}${cobradoSinCertHtml}${totalFilaFinal ? `<table class="t2" style="margin-top:14px"><colgroup><col/><col/></colgroup><tbody>${totalFilaFinal}</tbody></table>` : ""}${esCosto ? "" : `<div class="foot"><div class="sign"><div style="height:44px"></div><div class="ln">${cfg.nombre || "V+V Construcciones"}</div></div><div class="sign"><div style="height:44px"></div><div class="ln">${comitente} — Conforme</div></div></div>`}</div></body></html>`;
    setPdfHtml(html);
  }
  ...
  return (<div style={{ padding: "14px 16px 40px" }}>
    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 16, boxShadow: SHDsm }}>
      ...
      <div style={{ marginTop: 6 }}>
        {filas.length === 0 && <div style={{ fontSize: 12.5, color: T.muted, padding: "16px 0", textAlign: "center" }}>{todosPeriodos ? "Todavía no hay certificados." : `No hay certificados en ${mesLabel(mesSel)}.`}</div>}
...
function CertTab({ modo, obras, data, save, certsDe, indices }) {
  ...
  function imprimirCertificado(c) {
    ...
    const r = calcCert(c, obra, cs, indices); const ddTodos = detalleRubros(c, obra, cs); const dd = ddTodos.filter(d => d.per > 0);
...
function ResultadoTab({ obras, certs, certsDe, indices, data, save }) {
  ...
  let totFact = 0, totCobro = 0, totCostoDir = 0, totImpuestos = 0, totUtil = 0, totFijo = 0, totImprev = 0, provisCount = 0, provisMonto = 0, totBruto = 0, totCobroBruto = 0; const porObra = {}; const provisMeses = new Set();
  // Se arranca creando la entrada de las obras que YA TIENEN GASTOS cargados aunque
  // todavía no tengan certificados — así esos gastos no quedan afuera del cálculo
  ...
  // Obras marcadas como "histórico" (sin certificado): cobrado y ajuste salen solos de Caja/IPC.
  let totHistAjuste = 0;
  obras.forEach(o => {
