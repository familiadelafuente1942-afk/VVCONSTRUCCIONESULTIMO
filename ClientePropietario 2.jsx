...
function Panel({ obra, nombreCliente, tareas, auditoria, formularios, avance, renders, certif, envios, costos, onGuardarPropia, onCrearPropia, config, onGuardarConfig }) {
  ...
  return (<div style={{ minHeight: "100vh", background: T.bg }}>
    <div style={{ padding: "calc(20px + env(safe-area-inset-top)) 18px 40px" }}>
      ...
      {(() => {
        const fotosRecientes = (obra.fotos || []).slice().sort((a, b) => (b.ts || 0) - (a.ts || 0)).slice(0, 4);
        ...
        return (<>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 9 }}>
            ...
            <button onClick={() => setSeccion("galeria")} style={{ background: "none", border: "none", color: T.brass, fontWeight: 700, fontSize: 11.5, cursor: "pointer", padding: 0 }}>Ver todas</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6, marginBottom: 22 }}>
            {fotosRecientes.map((f, i) => (
              <button key={f.id || i} onClick={() => setSeccion("galeria")} style={{ padding: 0, border: "none", cursor: "pointer", borderRadius: 8, overflow: "hidden", aspectRatio: "1", background: T.bg }}>
