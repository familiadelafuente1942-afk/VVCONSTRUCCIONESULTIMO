  ...
  return (<div style={{ flex: 1, overflowY: "auto", paddingBottom: 90 }}>
    <div style={{ padding: "16px 20px" }}>
      ...
      <div style={{ background: T.navy, borderRadius: T.rsm, padding: "15px 17px", marginBottom: 20, borderBottom: `2px solid ${BRASS}` }}>
        ...
        {[["Contratado", contratado, "#fff"], ["Certificado", certificado, "#16A34A"], ["Saldo", contratado - certificado, BRASS]].map(([l, v, c], i) =>
      ...
      {obras.map(o => {
        ...
        return (<Card T={T} key={o.id} style={{ padding: 15, marginBottom: 11 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            ...
            <div style={{ flex: 2, background: T.bg, borderRadius: T.rsm, padding: "9px 11px" }}><div style={{ fontSize: 9.5, color: T.muted, textTransform: "uppercase" }}>Saldo pendiente</div><div style={{ fontSize: 12.5, fontWeight: 800, color: T.text, marginTop: 2 }}>{money(contr - cert)}</div></div>
          </div>
          ...
          {isOpen && <div style={{ marginTop: 12 }}>
            {(o.fotos || []).length > 0 && <div style={{ marginBottom: 12 }}><div style={{ fontSize: 10.5, fontWeight: 700, color: T.muted, textTransform: "uppercase", marginBottom: 7 }}>Avance fotográfico</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>{o.fotos.slice(0, 6).map((f, i) => <img key={i} src={f.url || f} alt="" style={{ width: "100%", aspectRatio: "1", objectFit: "cover", borderRadius: 6, border: `1px solid ${T.border}` }} />)}</div></div>}
